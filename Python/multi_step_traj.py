...
def estimate_step(t, nx,  nb_agents, X0, prior_noise,  swarm, meas_history, adjacency, vel, std_omega, std_v, std_range, f_range, f_odom):
    dynamics_noise = gtsam.noiseModel.Constrained.All(nx*nb_agents)#gtsam.noiseModel.Constrained.Sigmas(np.array([std_v * Delta_t, 0., std_omega * Delta_t] * nb_agents).reshape(nx * nb_agents,1))
