...
% load no_control_ERR_150_1_test_HighNoise_powell.mat
clear EST ERR TRUTH
load control_ERR_ring_150_1_test2_LowNoise_inv_cov_Linfty.mat
N = 20;
m = 5;
T = length(EST);
...
load control_ERR_ring_150_1_test2_LowNoise_inv_cov_L1.mat
% load control_ERR_ring_50_1_test2_Lowfreq_powell_trajopt_SAM.mat
N = 20;
m = 5;
T = length(EST);
...
load control_ERR_ring_150_1_test2_LowNoise_SAM.mat
N = 20;
m = 5;
T = length(EST);
...
clear EST TRUTH;
%load control_high_noise_full.mat%ERR_full_150.mat
load control_ERR_ring_150_1_test2_LowNoise_min_eig_SAM_final.mat
% load control_ERR_ring_50_1_test_Lowfreq_powell_trajopt_min_eig_SAM.mat
N = 20;
m = 5;
T = length(EST);
...
load control_ERR_ring_150_1_test2_LowNoise_inv_cov.mat
% load control_ERR_ring_50_1_test_Lowfreq_powell_trajopt_inv_cov.mat
N = 20;
m = 5;
T = length(EST);
...
% load control_ERR_ring_50_1_test_Lowfreq_powell_trajopt_det_inv_cov.mat
N = 20;
m = 5;
T = length(EST);
...
legend('Linfty invcov traj', 'L1 invcov traj',...
    'active guidance (min $(det($SAM$))^{-1}$)',  'SAM final step min eig',...
...
% legend('active guidance (min $(Tr($SAM$^{-1}))$)',...
%     'active guidance (min $(det($SAM$))^{-1}$)', 'active guidance (max $\lambda_{min}($SAM$)$)', ...
%     'active guidance (max $\lambda_{min}(P^{-1}_{EKF})$)', 'active guidance (min $(det(P^{-1}_{EKF}))^{-1}$)','location','best','interpreter','latex')
legend('Linfty invcov traj', 'L1 invcov traj',...
    'active guidance (min $(det($SAM$))^{-1}$)', 'SAM final step min eig', ...
    'active guidance (max $\lambda_{min}(P^{-1}_{EKF})$)', 'active guidance (min $(det(P^{-1}_{EKF}))^{-1}$)','location','best','interpreter','latex')
