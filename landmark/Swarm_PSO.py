...
class Swarm():
    ...
    def metric(self, w_set, states, optim_agent, METRIC):
        ...
        final_err = self.weight_tf * np.linalg.norm(
            states[0:2, -1, optim_agent].squeeze() - self.vehicles[optim_agent].target_point.squeeze())
        if METRIC == 'SAM':
            # print("Using SAM")
    ...
    def try_parallel(self, i,optim_agent,states,METRIC,U):
        ...
        for j in range(self.MPC_horizon):
            ...
            #propagate the state
                states[:,j+1:j+2,n] = U.discrete_step(states[:,j:j+1,n],inputs,self.pred_intv)
        final_err = self.weight_tf * np.linalg.norm(states[0:2,-1,optim_agent].squeeze() - self.vehicles[optim_agent].target_point.squeeze())
        if METRIC == 'SAM':
            # print("Using SAM")
            metrics = self.compute_SAM_metric2(states, all_inputs, METRIC)
        elif METRIC == 'min_eig_SAM':
            ...
            # print("Using obsv")
            metrics = self.compute_obsv_metric(states, np.expand_dims(self.w_set, axis=1), optim_agent)
        elif METRIC == 'min_eig_inv_cov':
            # print("Using inv_cov")
            metrics = -self.compute_inv_cov_metric(states, all_inputs, U, METRIC)
        elif METRIC == 'min_eig_inv_cov_appr':
    ...
    def compute_SAM_metric1(self, states, all_inputs,METRIC): #not used
        for i in range(self.MPC_horizon):
            state = states[:,i,:].squeeze()
            if i == 0:
                Jh = self.compute_range_jac(state)
    ...
    def compute_SAM_metric(self, states): #not used
        for i in range(self.MPC_horizon):
            state = states[:,i,:].squeeze()
            if i == 0:
                Jh = self.compute_range_jac(state)
