...
metrics = ['Trace_cov', 'Trace_inv_SAM']#, 'basic']#,'det_cov','SAM', 'basic']#, 'basic']#, 'SAM']
colorcode = 'rbgkcy'
linestyle = ['-.', '-',(0, (10, 10)), ':','--']
S = range(nb_agents)
...
for s in S:
    for i, metric in enumerate(metrics):
        ...
        if metric == 'basic':
            ...
            filename = str(MC_TOTAL) + '-' + str(nb_agents) + '-' + str(T) + '-' + adj_type + '-' + metric + '-' + str(
                s) + '-' + str(nb_landmarks) + '-once_TrajOpt.mat'
        ...
        for ai in range(nb_agents):
            ...
            plt.grid(True)
            plt.plot(posf[0, ai], posf[1, ai], color=colorcode[ai], marker='D')
            plt.ylim([-500, 3000])
            plt.xlim([0, 3000])
            plt.title(metric)
    plt.subplot(n_sub,1,1)
