...
class Swarm():
    ...
    def compute_obsv_metric(self, states, w_set, optim_agent):
        ...
        for j in range(self.MPC_horizon - 1):
            for n in range(self.nb_agents):
                ...
                if n == optim_agent:
                    inputs = np.array([[self.vehicles[n].v],[w_set[j,0]]])
                else:
                    inputs = np.array([[self.vehicles[n].v],[self.vehicles[n].omega]])
    ...
    def try_parallel(self, i,optim_agent,states,METRIC,U):
        ...
        if METRIC == 'SAM':
            ...
            # print("Using inv_cov")
            metrics = self.compute_inv_cov_metric(states, all_inputs, U, METRIC)
        elif METRIC == 'min_eig_inv_cov_traj_Linfty':
            metrics = self.compute_inv_cov_metric(states, all_inputs, U, METRIC)
        elif METRIC == 'min_eig_inv_cov_traj_L1':
            metrics = self.compute_inv_cov_metric(states, all_inputs, U, METRIC)
        else:
            print('metric not implemented')
        return metrics
    ...
    def Trajoptim(self, optim_agent, METRIC='obsv'):
        ...
        if optim_agent == None:
            ...
            w_set = np.random.randn((self.MPC_horizon))
    ...
    def compute_SAM_metric2(self, states, all_inputs, METRIC):
        ...
        for j in range(self.MPC_horizon - 1):
            ...
            for n in range(self.nb_agents):
                if n == 0:
                    ...
                    F = sc.linalg.block_diag(F, self.compute_individual_jac1(state_j[:, n:n + 1], inputs[:, n:n + 1],
                                                                             self.pred_intv))
            if j == 0:
                Qd = (self.pred_intv / self.Delta_t) * np.kron(np.eye(self.nb_agents),
                                                               self.vehicles[0].S_Q.T @ self.vehicles[0].S_Q)
            else:
                Qd = sc.linalg.block_diag(Qd, (self.pred_intv / self.Delta_t) * np.kron(np.eye(self.nb_agents),
                                                                                        self.vehicles[0].S_Q.T @
                                                                                        self.vehicles[0].S_Q))
        ...
        M_final = np.linalg.inv(M)[-self.nx * self.nb_agents:, -self.nx * self.nb_agents:]#J_final.T @ Q_final @ J_final
        # M = 0.5* (M + M.T)
        if METRIC == 'min_eig_SAM':
            return np.min(np.linalg.eigvals(M_final)).item()
        if METRIC == 'SAM':
            return 1 / np.linalg.det(M)
    ...
    def compute_SAM_metric1(self, states, all_inputs,METRIC):
        ...
        for j in range(self.MPC_horizon - 1):
            ...
            if j == 0:
                ...
                jd = self.compute_dynamics_jac1(state_j, inputs, j)
                Jd = np.vstack((Jd, jd))
        Qd = np.kron(np.eye(self.nb_agents * (self.MPC_horizon - 1)), (self.pred_intv/self.Delta_t)*(self.vehicles[0].S_Q.T @ self.vehicles[0].S_Q))
        J = np.vstack((Jh, Jd))
    ...
    def compute_inv_cov_metric(self,states,inputs,U, METRIC):
        ...
        if METRIC == 'min_eig_inv_cov_traj_Linfty':
            min_eig_val = []
        if METRIC == 'min_eig_inv_cov_traj_L1':
            min_eig_val = 0
        ...
        for i in range(self.MPC_horizon):
            ...
            for n in range(self.nb_agents):
                if n==0:
                    if i == 0:
                        ...
                        S_Q_block = self.S_Q
                    A =  U.dyn_jacobian(states[:,i:i+1,n], inputs[:,i:i+1,n], self.pred_intv)#self.compute_dynamics_jac1(state, input, i)#
                    ...
                    A = sc.linalg.block_diag(A, U.dyn_jacobian(states[:,i:i+1,n], inputs[:,i:i+1,n], self.pred_intv))#self.compute_dynamics_jac1(state, input, i))#
            ...
            # else:
                ...
                P_predict = P
                if METRIC == 'min_eig_inv_cov_traj_Linfty':
                    min_eig_val.append(np.min(np.linalg.eigvals(np.linalg.inv(P))).item())
                if METRIC == 'min_eig_inv_cov_traj_L1':
                    min_eig_val += np.min(np.linalg.eigvals(np.linalg.inv(P))).item()
