...
def init(swarmsize, x, y, theta, a_ids):
    ...
    for i in range(swarmsize-2):    # Number of drones (NUMBER_OF_DRONES - number of landmarks)
        x[i] = pos0[0][i] 
        y[i] = pos0[1][i] 
        a_ids[i] = i
        theta[i] = theta0[i]
    ...
    for i in range(2):              # Number of landmarks (which are technically drones that dont move)
        x[i+4] = landmarks[0][i]
        y[i+4] = landmarks[1][i]
        a_ids[i] = i
        theta[i] = 0
