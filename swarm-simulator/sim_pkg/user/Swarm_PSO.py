...
class Swarm():
    ...
    def add_vehicle(self, id, Delta_t, t, v, std_omega, std_v, std_range, f_range, f_odom, S_Q = np.diag([0.1, 0.1, 0.02])):
        self.id = id
        self.nb_agents += 1
        vehicle = Vehicle(self.id, Delta_t, t, v, std_omega, std_v, std_range, f_range, f_odom, S_Q)
    ...
    def Trajoptim(self, optim_agent, METRIC='obsv'):
        ...
        if optim_agent == None:
            ...
            for ind in range(NN):
                ...
                w_set = np.ones(self.MPC_horizon-1) * ws[ind]
    ...
    def compute_SAM_metric2(self, states, all_inputs, METRIC):
        ...
        if METRIC == 'Trace_inv_SAM':
            ...
            return np.linalg.norm(np.linalg.inv(L), 'fro') ** 2#np.trace(np.linalg.inv(M))#np.sum(1/np.linalg.eigvals(M)) #np.trace(np.linalg.inv(M))
        if METRIC == 'cond_SAM':
