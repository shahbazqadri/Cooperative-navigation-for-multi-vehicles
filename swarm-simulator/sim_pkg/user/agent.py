...
class Agent:
    ...
    class Unicycle:
        ...
        def find_u_jacobian(self, x, xp1, Delta_t):
            ...
            epsilon = 1e-8
            omega_delta_t_over_2 = omega * Delta_t / 2
            omega_delta_t_over_2 = np.where(np.abs(omega_delta_t_over_2) < epsilon, epsilon, omega_delta_t_over_2)

            sinc_term = np.sinc((omega * Delta_t) / (2 * np.pi))
            sinc_term = np.where(np.abs(sinc_term) < epsilon, epsilon, sinc_term)

            cos_theta_term = np.cos(theta + omega_delta_t_over_2)
            sin_theta_term = np.sin(theta + omega_delta_t_over_2)

            # partial v/ partial xp1
            jac1[0,0] = (1/Delta_t)/sinc_term * sin_theta_term
            jac1[0,1] = (1/Delta_t)/sinc_term * sin_theta_term

            numerator = -(xp1[0,:] - x[0,:]) * sin_theta_term + (xp1[1,:] - x[1,:]) * cos_theta_term
            denom = omega_delta_t_over_2

            fraction = (np.cos(omega_delta_t_over_2) - sinc_term) / denom
            fraction = np.where(np.abs(denom) < epsilon, 0, fraction)

            jac1[0,2] = (0.5 * (1/Delta_t)/sinc_term * numerator + mu / Delta_t * (- 0.5 / sinc_term ** 2 * fraction))
            
            jac0[0,0] = - jac1[0,0]
            jac0[0,1] = - jac1[0,1]
            
            jac0[0,2] = (0.5 * (1/Delta_t)/sinc_term * numerator - mu / Delta_t * (- 0.5 / sinc_term ** 2 * fraction))
            return jac0, jac1
